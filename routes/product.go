...
var (
	ErrPriceValue = errors.New("Price value invalid")
	ErrQuantityValue = errors.New("Quantity value invalid")
	ErrProductName = errors.New("Product name required")
)
func productsGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	utils.ExcuteTemplate(w, "product.html", struct {
		Total int64
...
func productCreateGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	message, alert := sessions.Flash(w, r)
	utils.ExcuteTemplate(w, "product-create.html", struct {
...
func productCreatePostHandler(w http.ResponseWriter, r *http.Request) {
	...
	sessions.Message("Added product", "success", w, r)
...
func productEditGetHandler(w http.ResponseWriter, r *http.Request) {
	key := r.URL.Query()
	productId, _ := strconv.ParseUint(key.Get("productId"), 10, 64)
	...
	utils.ExcuteTemplate(w, "product-edit.html", struct {
		Categories []models.Category
...
func productEditPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	sessions.Message(fmt.Sprintf("%d updated", rows), "info", w, r)
	http.Redirect(w, r, "/products", 302)
}
...
func productDeleteGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	sessions.Message(fmt.Sprintf("%d product was deleted", rows), "warning", w, r)
	http.Redirect(w, r, "/products", 302)
}